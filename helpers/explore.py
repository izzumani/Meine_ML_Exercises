...
def data_isnull(data,column_name,output_path=None):
    """output the general description of a  pandas Dataframe
       into a csv file
    
    """
    
    result = data.loc[data[column_name].isnull()]
    if output_path is not None:
        output = os.path.join(output_path,column_name + '_null.csv')
        result.to_csv(output)
        print('result saved at:', str(output))
    return result